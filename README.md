...
Integration of this feature is designed to be easy and compatible across harnesses - a "maintain" skill is added to skills.md Which the agent can pick up on. The maintain skill directs the agent to identify design decisions made during the session and persist it to the RULES.md file. It will also check if it's interpretation of any prompts contradicts the given project rules in RULES.md and either pick a solution that complies or point out that your intent clashes with pre-existing rules. The rules in question will be struck down or upheld based on the user's verdict.  
